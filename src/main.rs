...
async fn post(client: Client, args: &Post) -> anyhow::Result<()> {
    ...
    for pair in args.body.iter() {
        match pair.t {
            ...
            KvPairType::Header => {
                if let Ok(k) = HeaderName::from_str(pair.k.as_str()) {
                    if let Ok(v) = HeaderValue::from_str(pair.v.as_str()) {
...
fn print_body(mine: Option<Mime>, resp: &String) {
    match mine {
